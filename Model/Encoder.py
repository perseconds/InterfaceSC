...
LastEditors: kwz
LastEditTime: 2025-06-03 14:36:35
Description: SwinTranformer作为编码器
'''
...
class SourceEncoder(nn.Module):
    def __init__(self, img_size, patch_size, in_chans,
        ...
        self.patch_embed = PatchEmbed(img_size, patch_size=2, in_chans=3, embed_dim=embed_dims[0])
    ...
    def forward(self, x):
        B, C, H, W = x.size()
        device = x.get_device()
        x = self.patch_embed(x) # patch大小为2
        # print(x.shape)
...
if __name__ == "__main__":
    ...
    encoder = create_encoder(**encoder_kwargs)
    # encoder.update_resolution(32, 32)
